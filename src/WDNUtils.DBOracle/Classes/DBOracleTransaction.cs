...
{
    /// <summary>
    /// Oracle transaction/savepoint wrapper class
    /// </summary>
    public sealed class DBOracleTransaction
    {
        #region Logger
        ...
        /// </summary>
        private DBOracleConnection Connection { get; set; }
        ...
        internal DBOracleTransaction(DBOracleConnection connection, string savepointName)
        {
            ...
            lock (transactionLock)
            {
                ...
                try
                {
                    if (Connection.Transaction is null)
                        throw new InvalidOperationException(DBOracleLocalizedText.DBOracleTransaction_Commit_AlreadyFinished);
                ...
                catch (Exception ex)
                {
                    try
                    {
                        Log.Error(DBOracleLocalizedText.DBOracleTransaction_Commit_Error, ex);
                    }
                    catch (Exception)
                    {
                        // Nothing to do
            ...
            lock (transactionLock)
            {
                ...
                try
                {
                    if (Connection.Transaction is null)
                        throw new InvalidOperationException((SavepointName is null)
                            ? DBOracleLocalizedText.DBOracleTransaction_RollbackTransaction_AlreadyFinished
                            : string.Format(DBOracleLocalizedText.DBOracleTransaction_RollbackSavepoint_AlreadyFinished, SavepointName));
                    ...
                    try
                    {
                        ...
                        {
                            Connection.Transaction.Rollback(savepointName: SavepointName);
                        }
                ...
                catch (Exception ex)
                {
                    try
                    {
                        ...
                        {
                            Log.Error(string.Format(DBOracleLocalizedText.DBOracleTransaction_RollbackSavepoint_Error, SavepointName), ex);
                        }
                    }
                    catch (Exception)
                    {
